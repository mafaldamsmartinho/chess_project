...
class Bot:
    ...
    def list_possible_scored_moves(self, game: Game) -> list[dict]:
        ...
        for i, start_p in enumerate(bot_positions):
            ...
            for j, end_p in enumerate(end_positions):
                valid_piece_move = self.is_valid_piece_move(game=game, piece=piece, start_p=start_p, end_p=end_p)
                if valid_piece_move and is_legal_move(game=game, start_position=start_p, end_position=end_p):
                    if end_p not in kill_points_dict:  # Avoid duplicate checks of same piece in repeted end positions
                        kill_points_dict[end_p] = self.kill_piece_points(game=game, end=end_p)
                    move_score = self.score_move(game=game,
                                                 start_position=start_p,
                                                 end_position=end_p,
                                                 kill_points=kill_points_dict[end_p])
                    scored_moves.append({"start_square": start_p,
                                         "end_square": end_p,
                                         "score": move_score})
        return scored_moves

    def number_of_possible_piece_moves(self, game: Game, piece_position: tuple, end_positions: list) -> int:
        moves = 0
        moved_piece = game.board.get_piece_by_index(piece_position)
        for end_p in end_positions:
            valid_piece_move = self.is_valid_piece_move(game=game, piece=moved_piece, start_p=piece_position, end_p=end_p)
            if valid_piece_move and is_legal_move(game=game, start_position=piece_position, end_position=end_p):
                moves += 1
        return moves

    def is_valid_piece_move(self, game: Game, piece: Piece, start_p: tuple, end_p: tuple) -> bool:
        match piece.type:
            case PieceType.KING:
                valid_piece_move = validate_king_move(board=game.board,
                                                            start_position=start_p,
                                                            end_position=end_p)
            case PieceType.QUEEN:
                valid_piece_move = validate_queen_move(board=game.board,
                                                            start_position=start_p,
                                                            end_position=end_p)
            case PieceType.KNIGHT:
                valid_piece_move = validate_knight_move(board=game.board,
                                                            start_position=start_p,
                                                            end_position=end_p)
            case PieceType.BISHOP:
                valid_piece_move = validate_bishop_move(board=game.board,
                                                            start_position=start_p,
                                                            end_position=end_p)
            case PieceType.ROOK:
                valid_piece_move = validate_rook_move(board=game.board,
                                                            start_position=start_p,
                                                            end_position=end_p)
            case PieceType.PAWN:
                valid_piece_move = validate_pawn_move(board=game.board,
                                                            start_position=start_p,
                                                            end_position=end_p)
    ...
    def expose_pieces_points(self, game: Game, bot_virtual_position: tuple, end_positions: list) -> int:
        ...
        for opponent_p in end_positions:
            ...
            if opponent_piece is not None:
                valid_piece_move = self.is_valid_piece_move(game=game, piece=opponent_piece, start_p=opponent_p, end_p=bot_virtual_position)
                if valid_piece_move and is_legal_move(game=game, start_position=opponent_p, end_position=bot_virtual_position):
    ...
    def pieces_protected(self, game: Game, bot_positions: list, end_positions: list) -> int:
        ...
        for protected_square in bot_positions:
            protected_piece = game.board.get_piece_by_index(index_position=protected_square)
    ...
    def score_move(self, game: Game, start_position: tuple, end_position: tuple, kill_points: int) -> float:
        ...
        move_score = self.score_move_function(game=game,
                                              start_position=start_position,
                                              end_position=end_position,
                                              kill_points=kill_points)
        game.board.move_piece(start=end_position,
                              end=start_position)
    ...
    def score_move_function(self, game: Game, start_position: tuple, end_position: tuple, kill_points: int) -> float:
        ...
        points_pieces_protected = self.pieces_protected(game=game,
                                                    bot_positions=bot_positions,
                                                    end_positions=end_positions)
        points_exposed_pieces = self.expose_pieces_points(game=game,
                                                          bot_virtual_position=end_position,
                                                          end_positions=end_positions)
        points_check_opponent_king = self.check_opponent_king_points(game=game)
        score = (points_possible_moves/20) * 0.2 + points_pieces_protected * 0.2 + points_exposed_pieces * 0.2 + kill_points * 0.2 + points_check_opponent_king * 0.2
        return score
