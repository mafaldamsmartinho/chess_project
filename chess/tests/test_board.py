...
class TestInitSetupBoard:
    def test_init_board_size_when_setup(self, board):
        assert len(board.board) == 8
        assert all(len(row) == 8 for row in board.board)

    def test_correct_number_pieces_when_setup(self, board):
        pieces_number = sum(el is not None for row in board.board for el in row)
        assert pieces_number == 32

    def test_correct_colours_when_setup(self, board):
        white_pieces = 0
        black_pieces = 0
...
class TestIsValidPosition:
    ...
    def test_returns_bool_when_valid_position(self, position, expected, board):
...
class TestIsValidPositionIndex:
    ...
    def test_returns_bool_when_valid_position_index(self, position, expected, board):
        assert board.is_valid_position_index(index_position=position) == expected


class TestPositionToIndex:
    @pytest.mark.parametrize("position, expected", [("a2", (1, 0)), ("b6", (5, 1))])
    def test_position_to_index_valid(self, position, expected, board):
        assert board.position_to_index(position=position) == expected

    def test_position_to_index_invalid(self, board):
...
class TestIndexToPosition:
    ...
    def test_index_to_position_valid(self, index_position, expected, board):
        assert board.index_to_position(index_position=index_position) == expected

    def test_index_to_position_invalid_column(self, board):
        with pytest.raises(IndexError):
            board.index_to_position(index_position=(0, 8))


class TestGetPiece:
    def test_get_piece_occupied_square(self, board):
    ...
    def test_get_piece_empty_square(self, board):
        assert board.get_piece(position="a3") is None

    def test_get_piece_invalid_position(self, board):
        with pytest.raises(ValueError):
            board.get_piece(position="z9")


class TestGetPieceByIndex:
    def test_get_piece_by_index_occupied_square(self, board):
    ...
    def test_get_piece_by_index_empty_square(self, board):
        assert board.get_piece_by_index(index_position=(3, 3)) is None


class TestSetPiece:
    def test_set_piece_places_piece(self, board):
...
class TestMovePiece:
    def test_move_piece_reaches_destination(self, board):
    ...
    def test_move_piece_start_becomes_empty(self, board):
        board.move_piece(start=(1, 0), end=(2, 0))

        assert board.get_piece_by_index(index_position=(1, 0)) is None

    def test_move_piece_captures_or_overwrites_destination(self, board):
    ...
    def test_move_piece_empty_start_makes_destination_empty(self, board):
...
class TestToDict:
    def test_to_dict_has_board_shape(self, board):
        board_dict = board.to_dict()

        assert len(board_dict) == 8
        assert all(len(row) == 8 for row in board_dict)

    def test_to_dict_converts_piece(self, board):
    ...
    def test_to_dict_keeps_empty_squares(self, board):
        assert board.to_dict()[3][3] is None

    def test_to_dict_returns_expected_values_after_board_change(self, board):
